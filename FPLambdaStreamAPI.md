...
и возвращает ее результат также в виде объекта типа `T`.
    ...
    Отличие от BiFunction в том, что оба аргумента и тип возмвращаемого значения совпадают
...
Этот пример создает новый экземпляр AddThree и назначает его переменной Function.
Вызывается метод apply() для экземпляра AddThree.
Распечатывает результат (который равен 7).
...
https://javahelp.online/osnovy/funktsionalnye-interfeysy-java
...
Агрегатные функции (агрегации) — это функции, которые вычисляются от группы значений и объединяют их в одно результирующее. Если в поле с измерением добавить агрегацию, то поле становится показателем.
...
подробнее https://habr.com/ru/post/346782/ , https://megahub.me/post/16
