...
отличие iterator от listIteror не только в том, что listIterator позволяет проходиться в обратном порядке, но и в том, что
listIterator может изменять объект, например добавить или удалить...
...
![img](https://github.com/swebdesigne/TheoryForCourse/blob/main/src/main/resources/list.png)
...
![img](https://github.com/swebdesigne/TheoryForCourse/blob/main/src/main/resources/linkedlist.png)
...
Как упоминалось ранее, существуют 3 основные реализации интерфейса Map. 
У каждой из них есть свои особенности:
Порядок элементов. HashMap и Hashtable не гарантируют, что элементы будут храниться в порядке добавления. 
Кроме того, они не гарантируют, что порядок элементов не будет меняться со временем. 
...
*** Поскольку HashMap не имплементируется от iterable - это заставляет использовать такие инутсрументы как keySet() для работы с объектом в цикле.
просто пройтись в цикле и получить по индексу объект у нас не получится.
...
Для списка необходимо переопределять equals и hashCode потому что список имеет метод equals.
...
Stack в java, как пример, осуществляется в стеке вызовов.
...
*** Если сделать коофициент загрузки меньше 0.75 то мы получим частое увеличене нашей мапы, а это трудоемкий процесс, что приведет к потере производительности.
если фактор загрузки больше 0.75 тогда наши баккеты будут иметь много элементов, что тоже приведет к потере производительности. 
...
1. Двоичное дерево - это структура данных, в которой каждый родительский узел может иметь максимум 2 дочерних элемента.

Двоичное дерево поиска - это двоичное дерево, в котором левый дочерний элемент содержит только узлы со значениями, меньшими или 

равными родительскому узлу, а правый дочерних элемент содержит узлы со значениями равным или превышающим родительский узел.

2. в узел можно прийти только одним единственным путем

3. бинарное дерево не имеет определенного порядка расположения элементов данных.

4. левый и правый дочерние узлы отвечают условиям равенства > = <  
